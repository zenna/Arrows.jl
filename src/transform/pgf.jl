...
function pgfreplace(carr::CompArrow,
                    sarr::SubArrow,
                    tparent::TraceParent,
                    abtvals::TraceAbVals;
                    pgf=pgf)
...
function pgfreplace(parr::PrimArrow, sarr::SubArrow, tparent::TraceParent, abtvals::TraceAbVals; pgf=pgf)
  idabvals = tarr_idabv(TraceSubArrow(tparent, sarr), abtvals)
  pgf(parr, sarr, idabvals)
end

"""
Parameter generating function (pgf) of `carr`
...
function pgf(arr::CompArrow,
  ...
  function custpgfreplace(arr, sarr, tparent, abtvals)
